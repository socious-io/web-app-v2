...
export async function getToReviewList(payload: {
  id: string;
  page: number;
}): Promise<Pagination<UserApplicantResp[]>> {
  return get(
    `projects/${payload.id}/applicants?limit=100&status=PENDING&page=${payload.page}`
  ).then(({ data }) => data);
}

export async function getDeclinedList(payload: {
  id: string;
  page: number;
}): Promise<Pagination<UserApplicantResp[]>> {
  return get(
    `projects/${payload.id}/applicants?limit=100&status=REJECTED&page=${payload.page}`
  ).then(({ data }) => data);
}

export async function getHiredList(payload: { id: string; page: number }): Promise<MissionsResp> {
  return get(
    `projects/${payload.id}/missions?limit=100&filter.status=ACTIVE,COMPLETE&page=${payload.page}`
  ).then(({ data }) => data);
}

      ...
      <div className="w-full flex flex-col">
        ...
        >
          <div className="flex flex-col w-full md:w-[512px]">
            <span className="font-semibold text-sm text-Gray-light-mode-700">{translate('item.label')}</span>
            <span className="font-normal text-sm text-Gray-light-mode-600">{translate('item.description')}</span>
          </div>
            ...
            {item.toggleButtons.map((btn, index) => (
              <div key={`label-toggle-${index}`} className={css.item}>
                ...
                <div className="flex flex-col">
                  <p className={css.title}>{translate(btn.text)}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <>
      <div>
        <div className="w-full pb-8 items-center">
          <h2 className="grow css.title text-lg font-semibold">{translate('notification.title')}</h2>
          <p className="text-sm font-normal text-Gray-light-mode-600 pt-1">{translate('notification.description')}</p>
        </div>
      </div>
      ...
      <Button variant="contained" color="primary" customStyle="w-full md:w-fit mr-0 ml-auto" onClick={onSave}>
        {translate('notification.saveButton')}
      </Button>
    </>
  );
};

...
export const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const { translate, getRootProps, getInputProps, subtitle, errorMessage, fileName } = useFileUploader(
    files,
    onDropFiles,
    fileTypes,
    maxSize,
  ...
  return (
    <>
      ...
      {showPreviewImages && !!files.length && (
        <div className={styles['row']}>
          {files.map(file => (
            <div key={file.id} className={styles['preview']}>
              <img src={URL.createObjectURL(file.file)} className={styles['preview__image']} />
              <div className={styles['preview__delete']} onClick={() => file?.id && onDeleteFiles?.(file.id)}>

...
import { Contract, createContractAdaptor, depositContractAdaptor, Service } from 'src/core/adaptors';
import {
  Card,
  CurrentIdentity,
  OrgMeta,
...
export const useServicePaymentFlow = () => {
  ...
  const { isConnected, open: openConnect, account, signer, chainId } = Dapp.useWeb3();
  ...
  const handleCryptoPayment = async (contract: Contract) => {
    try {
      ...
      const result = await Dapp.escrow({
        signer,
        chainId,
        //FIXME: needs contract.amounts for total amount
        totalAmount: contract.price || 0,
        escrowAmount: contract.price || 0,
        contributor,
  ...
  const onSubmit = formData => {
    //TODO: BE new endpoint for submit requirements

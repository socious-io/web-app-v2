...
export function getContractStatus(
  ...
  let status: SemanticContractStatus = 'Closed';
  if (isCurrentIdentityProvider) {
    switch (contractStatus) {
      case 'CREATED':
        if (hasPayment) status = 'Awaiting confirmation';
        else status = 'Offer sent';
        break;
      case 'SIGNED':
        if (hasPayment) status = 'Ongoing';
        else if (paymentType === 'PAID') status = 'Payment required';

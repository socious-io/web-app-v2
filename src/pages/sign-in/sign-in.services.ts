...
export async function setAuthCookies(loginResp: LoginResp) {
  await nonPermanentStorage.set({ key: 'access_token', value: loginResp.access_token });
  await nonPermanentStorage.set({ key: 'refresh_token', value: loginResp.refresh_token });
  await nonPermanentStorage.set({ key: 'token_type', value: loginResp.token_type });
}

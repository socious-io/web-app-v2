...
import { API_SERVER, API_SERVER_V3, APP_URL, VALID_EMAIL, VALID_PASSWORD } from '../authentication/constants';
...
describe('Like Feed items', () => {
    ...
    beforeEach(() => {
        ...
        cy.intercept('POST', 'https://pulse.walletconnect.org/e?projectId=*&st=*&sv=*',
            req => {
                req.reply(200);
            }
        );
        cy.intercept('GET', 'https://api.web3modal.org/getAnalyticsConfig?projectId=*&st=*&sv=*',
            req => {
                req.reply(200);
            }
        );
    ...
    it('should log in user from the intro page using Socious ID', () => {
        ...
        cy.origin('https://dev-id.socious.io',
            {
                args: {
                    email: VALID_EMAIL,

import { APP_URL, FIRSTNAME, INVALID_EMAIL, INVALID_PASSWORD, LASTNAME, USERNAME, VALID_EMAIL, VALID_PASSWORD, VERIFICATION_CODE, WRONG_EMAIL } from "./constants";

describe('user signs up to the platform', () => {
    beforeEach(() => {

        cy.intercept('POST', `${APP_URL}/batch?projectId=*&st=events_sdk&sv=*`,
            req => {
                req.reply(200);
            }
        ).as('postProject');
        ...
        cy.intercept('GET', `${APP_URL}/getDesktopListings?projectId=*&sdkType=wcm&sdkVersion=js-2.7.0&page=1&entries=9&version=2`,
            req => {
                req.reply(200);
            }
        ...
        cy.intercept('POST', `${APP_URL}/getDesktopListings?projectId=&sdkTyp`,
            req => {
                req.reply(200);
            }
        );
        cy.intercept('POST', `${APP_URL}/sign-up/user/auth/register?event_id=`,
            req => {
                req.reply(200);
            }
        ).as('postRegister');
        ...
        cy.intercept(
            'GET',
            `${APP_URL}/sign-up/user/auth/otp/confirm?t=*&email=VALID-EMAIL%40GMAIL.COM&code=*`,
            (req) => {
                if (req.url.includes('code=000000')) { // Simulating incorrect OTP
                    req.reply({
                        statusCode: 400,
        ...
        cy.intercept('GET', `${APP_URL}/getDesktopListings?projectId=*&sdkType=*&sdkVersion=*&page=1&entries=9&version=2`,
            req => {
                req.reply(200);
            }
        );
        ...
        cy.intercept('GET', `${APP_URL}/getDesktopListings?projectId=*&sdkTy`,
            req => {
                req.reply(200);
            }
        );
        cy.intercept('GET', `https://explorer-api.walletconnect.com/w3m/v1/getDesktopListings?projectId=*&sdkTyp`,
            req => {
                req.reply(200);
            }
        );
        ...
        cy.intercept('GET', `https://pulse.walletconnect.org/batch?projectId=*&st=events_sdk&sv=js-2.17.0`,
            req => {
                req.reply(200);
            }
        );
        ...
        cy.intercept('POST', `${APP_URL}/sign-up/user/user/change-password-direct`,
            req => {
                req.reply(200);
            }
        );
        cy.intercept('GET', `${APP_URL}/sign-up/user/user/profile?t=*`,
            req => {
                req.reply(200);
            }
        );

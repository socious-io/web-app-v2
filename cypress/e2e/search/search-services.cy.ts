...
import { ORGS_DETAIL, SEARCH_ORGANIZATIONS, SEARCH_PEOPLE, SEARCH_RESULT, SEARCH_RESULT_DETAIL, SEARCH_SERVICES, SEARCHED_JOBS } from "./mocks";

const SIGNINGUP_EMAIL = generateRandomEmail();
const user = new User(FIRSTNAME, LASTNAME, SIGNINGUP_EMAIL, USERNAME);

const SEARCH_KEYWORD = 'Logo';
...
describe('search bar test automation for jobs', () => {
    ...
    it('user recieves an empty search result', () => {
        returnEmpty = true;
        cy.visit(`${APP_URL}/jobs`);
        cy.get('#search-input').click();
        cy.contains('div', 'Services').click();

        cy.get('#search-modal').type('something-random');
        cy.wait('@mockSearch');

        cy.contains('No services found').should('be.visible');
        cy.get('[data-testid="search-result"]').should('not.exist');
    });
    it('user clicks on search bar, chooses services, and gets the searched data inside the modal', () => {
        returnEmpty = false;
        cy.visit(`${APP_URL}/jobs`);
        ...
        cy.get('#search-modal').type(`${SEARCH_KEYWORD}`);

        cy.get('[data-testid="search-result"]').first().should('exist');
    });
    it('user navigates to jobs page and opens the search Services modal and clicks see all results', () => {
        returnEmpty = false;
        cy.visit(`${APP_URL}/jobs`);
        cy.url().should('contain', '/jobs');
        cy.contains('Find work that matters to you and the world').should('be.visible');
        ...
        cy.contains(`Search for ${SEARCH_KEYWORD}`).should('exist');
    });
    it('user navigates to jobs page and opens the search Servies modal', () => {
        returnEmpty = false;
        cy.visit(`${APP_URL}/jobs`);
        cy.url().should('contain', '/jobs');
        cy.contains('Find work that matters to you and the world').should('be.visible');
